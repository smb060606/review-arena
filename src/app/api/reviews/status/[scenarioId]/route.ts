...
import { getCodeRabbitCheckRuns, getCopilotReviews, getCodeRabbitComments } from "@/lib/github";
...
export async function GET(
  ...
  try {
    if (scenario.coderabbitPrNumber) {
      const checkRun = await getCodeRabbitCheckRuns(scenario.coderabbitPrNumber);
      if (checkRun) {
        ...
        const comments = await getCodeRabbitComments(scenario.coderabbitPrNumber);
  ...
  try {
    if (scenario.copilotPrNumber) {
      const reviews = await getCopilotReviews(scenario.copilotPrNumber);
  ...
  const status: ScenarioStatus = {
    coderabbit: {
      prNumber: scenario.coderabbitPrNumber,
      status: crStatus,
      commentCount: crCommentCount,
      checkRunId: crCheckRunId,
    },
    copilot: {
      prNumber: scenario.copilotPrNumber,
